...
-   **경로 파라미터:** 없음
-   **쿼리 파라미터:** 
    - `skip` (선택사항): 건너뛸 노트북 수 (기본값: 0)
    - `limit` (선택사항): 반환할 최대 노트북 수 (기본값: 100, 최대: 100)
-   **요청 본문:** 없음
...
-   **쿼리 파라미터:** 
    - `q` (필수): 검색어 (최소 1자)
    - `limit` (선택사항): 반환할 최대 결과 수 (기본값: 20, 최대: 50)
-   **요청 본문:** 없음
-   **응답 본문:** `List[NotebookSchema]`

### 3. 캐시 클리어

-   **엔드포인트:** `POST /cache/clear`
-   **설명:** 노트북 관련 캐시를 클리어합니다.
-   **경로 파라미터:** 없음
-   **쿼리 파라미터:** 없음
-   **요청 본문:** 없음
-   **응답 본문:** 
...
-   **엔드포인트:** `GET /admin/performance-stats`
-   **설명:** 데이터베이스 성능 통계를 조회합니다.
-   **경로 파라미터:** 없음
-   **쿼리 파라미터:** 없음
-   **요청 본문:** 없음
-   **응답 본문:** 
    ```json
    {
        "database_stats": {
            ...
            "files_count": 100,
            "contents_count": 50,
            "indexes": ["idx_notebooks_title", "..."]
        },
        "cache_info": {
            "notebooks_cache": {"hits": 10, "misses": 2, "maxsize": 128, "currsize": 5},
            "summary_cache": {"hits": 15, "misses": 3, "maxsize": 256, "currsize": 8}
        }
    }
    ```
...
-   **쿼리 파라미터:** 
    - `summary_only` (선택사항): 요약 정보만 반환 (기본값: false)
-   **요청 본문:** 없음
...
-   **쿼리 파라미터:** 
    - `summary_only` (선택사항): 챕터 요약 정보만 반환 (기본값: false)
-   **요청 본문:** 없음
